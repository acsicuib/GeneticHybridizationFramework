...
import ast
import copy
from resopt.param.default import *
...
# Pymoo hybrid problem solving
parser_hybrid_solve = subparsers.add_parser('hybrid_solve', help='Hybrid solve optimization problem')

parser_hybrid_solve.add_argument('-i', '--input', type=argparse.FileType('rb'), help='Input file path used for generating the network')
parser_hybrid_solve.add_argument('--objectives', type=str, nargs='+', default=['distance','nodes'], help='Objectives to use in the algorithm')

parser_hybrid_solve.add_argument('--pop_size', type=int, default=HYBRID_POP_SIZE, help='Population size')
parser_hybrid_solve.add_argument('--algorithms', nargs='+', type=str, default=HYBRID_ALGORITHMS, help='Name of the algorithms to be used to hybrid solve the problem')

parser_hybrid_solve.add_argument('--termination_type', type=str, default=TERMINATION_TYPE, help='Termination type for the algorithm')
parser_hybrid_solve.add_argument('--n_gen', type=int, default=HYBRID_N_GEN, help='Number of generations as termination parameter')
parser_hybrid_solve.add_argument('--gen_steps', nargs='+', default=HYBRID_GEN_STEPS, type=int, help='Generation transition steps for the dumping of solutions')

parser_hybrid_solve.add_argument('--sol_dumping', type=type_point_list, default=HYBRID_SOL_DUMPING, help='Instructions on how to dump the solutions on each generation transition step')

parser_hybrid_solve.add_argument('--n_replicas', type=int, default=N_REPLICAS, help='Max number of replicas for service assignment')
parser_hybrid_solve.add_argument('--mutation_prob_move', type=float, choices=[Range(0.0, 1.0)], default=MUTATION_PROB_MOVE, help='Probability of mutation of type move in task assignment to nodes')
parser_hybrid_solve.add_argument('--mutation_prob_change', type=float, choices=[Range(0.0, 1.0)], default=MUTATION_PROB_CHANGE, help='Probability of mutation of type change in task assignment to nodes')
parser_hybrid_solve.add_argument('--mutation_prob_binomial', type=float, choices=[Range(0.0, 1.0)], default=MUTATION_PROB_BINOMIAL, help='Binomial probability which will give the amount of mutations for each individual depending on infrastructure size. Only for mutation versions that support it (currently, only Mutation_v4)')

parser_hybrid_solve.add_argument('-v', '--verbose', action='store_true')
parser_hybrid_solve.add_argument('--print', action='store_true', help='Print on console useful information about the generated network')

parser_hybrid_solve.add_argument('-o', '--output', nargs='*', type=argparse.FileType('w'), help='Output file paths used for storing the solution data')
parser_hybrid_solve.add_argument('--save_history', action='store_true', help='Will save history to retrieve the evolution of the solutions')

parser_hybrid_solve.add_argument('--single_mode', action='store_true', help='Will force the algorithm to use single mode optimization')
parser_hybrid_solve.add_argument('-l', '--lmb', type=float, choices=[Range(0.0, 1.0)], default=LAMBDA, help='Parameter used for weighted average when converting bimode to single-mode.')

# Helper for ILP constraint handling
parser_hybrid_solve.add_argument('--o1_max', type=float, default=None, help='Will use this value as a max value constraint for O1')
parser_hybrid_solve.add_argument('--o2_min', type=float, default=None, help='Will use this value as a min value constraint for O2')

# Specific parameter for algorithms that need reference directions
parser_hybrid_solve.add_argument('--n_partitions', type=int, default=N_PARTITIONS, help='Specific parameter for algorithm NSGA3. Set the number of partitions for reference directions.')

# Specific parameter for algorithms that need reference point
parser_hybrid_solve.add_argument('--ref_points', type=type_point_list, default=REF_POINTS, help='Specific parameter for algorithms that requiere reference points')

# Problem operators
parser_hybrid_solve.add_argument('--sampling_version', type=int, default=SAMPLING_VERSION, help='Sampling version number to be used')
parser_hybrid_solve.add_argument('--crossover_version', type=int, default=CROSSOVER_VERSION, help='Crossover version number to be used')
parser_hybrid_solve.add_argument('--repair_version', type=int, default=0, help='Repair version number to be used')
parser_hybrid_solve.add_argument('--mutation_version', type=int, default=MUTATION_VERSION, help='Mutation version number to be used')
...
parser_analyze.add_argument('--ref_points', type=type_point_list, default=None, help='Specific parameter for algorithms that requiere reference points')

parser_analyze.add_argument('--entire_population', action='store_true', help='Files contains all the entire population and a column to indicate whether it\'s part of the solution or not.')

parser_analyze.add_argument('--alg_names', nargs='*', type=str, default=[ALGORITHM], help='Algorithm names to use on the table')
parser_analyze.add_argument('--pop_values', nargs='*', type=int, default=[POP_SIZE], help='Population values to use on the table')
parser_analyze.add_argument('--seed_values', nargs='*', type=int, default=[SEED], help='Seed values used for calculating the algorithms to use on the table')
parser_analyze.add_argument('--gen_values', nargs='*', type=int, default=[0], help='Generation step values used for printing the evolution of each indicator')
...
parser_analyze.add_argument('--n_algorithms', type=int, default=0, help='Number of algorihtms within the solution file (hybrid solve for calculating genetic load).')
parser_analyze.add_argument('--objectives', type=str, nargs='+', default=['distance','nodes'], help='Objectives to use in the algorithm')
parser_analyze.add_argument('--gen_steps', type=int, nargs='+', default=[], help='Generation transition steps for the dumping of solutions')

parser_analyze.add_argument('--print', action='store_true', help='Print on console the table')
parser_analyze.add_argument('-o', '--output', type=argparse.FileType('w'), help='Output file path used for saving generated table (hybrid solve for calculating genetic load)')
...
parser_plot.add_argument('--max_gen', type=int, default=0, help='Max generation to plot for history plotting')

parser_plot.add_argument('--hybrid_history', action='store_true', help='Plot a single solution including the history representing the evolution and the genetic load with the form of a scatter plot')
parser_plot.add_argument('--hybrid_legend', nargs='*', type=str, help='List of names for the different inputs to be plotted and added to the legend')
parser_plot.add_argument('--gen_steps', nargs='+', default=HYBRID_GEN_STEPS, type=int, help='Generation transition steps for the dumping of solutions')

parser_plot.add_argument('--stack', action='store_true', help='Plot stack of genetic loads for hybrid algorithm')
...
if __name__ == '__main__':
    ...
    if configs.command == 'hybrid_solve':
        conf_copy = [copy.deepcopy(configs) for _ in range(len(configs.algorithms))]
        for i in range(len(configs.algorithms)):
            conf_copy[i].algorithm = configs.algorithms[i]
