...
def plot_3d_scatter(algorithm,it,file_out,do_gif=False):
    # it = 1
    # algorithm = "NSGA2"
    df = load_data_normalized(path_exp,file,it,algorithm)
    print(df.head())
...
def plot_metrics_time_series(file_metrics,file_hybrids=None):
    ...
    for replica in range(1,N_EXECUTIONS+1):
        ...
        if file_hybrids is not None:
            df_hybrids = pd.read_csv(file_hybrids.format(algorithm=ALGORITHMS[0],seed=replica,HYBRID_POP_SIZE=HYBRID_POP_SIZE,HYBRID_N_GEN=HYBRID_N_GEN))
        else:
            df_hybrids = None
...
file_hybrids = "{algorithm}_{seed}_{HYBRID_POP_SIZE}-{HYBRID_N_GEN}_SV0-CV2-MV1_MM0.2-MC0.1-MB0.1.txt"


file_metrics = middle_path_ana_exp + f"/table_{ALGORITHMS[-1]}_{POP_SIZE}:{N_GEN}.csv"
# file_hybrids = "hybrids/table_merge.csv"


seed = range(1,N_EXECUTIONS+1)
ALGORITHMS = ['NSGA2','NSGA3','UNSGA3','SMSEMOA']
...
if __name__ == "__main__":
    ...
    plot_metrics_time_series(file_metrics, file_hybrids=None)